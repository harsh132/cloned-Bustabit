...
exports.register = function (req, res, next) {
    ...
    if (username.length > 16) return res.render("register", { warning: "Username is too long", values: values.user });
    ...
    console.log("Registering user", username, password, password2, email);
...
exports.login = function (req, res, next) {
    ...
    if (!username || !password) return res.render("login", { warning: "no username or password" });
...
exports.enableMfa = function (req, res, next) {
    ...
    if (!lib.validateSignature(user.username + "|" + secret, sig)) return next("Could not validate sig");
...
exports.sendPasswordRecover = function (req, res, next) {
    ...
    database.getUsersFromEmail(email, function (err, users) {
        if (err) {
            ...
            else return next(new Error("Unable to get users by email " + email + ": \n" + err));
        }
        ...
        async.each(
            ...
            function (err) {
                ...
                sendEmail.passwordReset(email, recoveryList, function (err) {
                    if (err) return next(new Error("Unable to send password email: \n" + err));
...
exports.handleWithdrawRequest = function (req, res, next) {
    ...
    var r = /^[1-9]\d*(\.\d{0,2})?$/;
    if (!r.test(amount)) return res.render("withdraw-request", { user: user, id: uuid.v4(), warning: "Not a valid amount" });
    ...
    if (!password) return res.render("withdraw-request", { user: user, id: uuid.v4(), warning: "Must enter a password" });

    if (!lib.isUUIDv4(withdrawalId)) return res.render("withdraw-request", { user: user, id: uuid.v4(), warning: "Could not find a one-time token" });
    ...
    database.validateUser(user.username, password, otp, function (err) {
        if (err) {
            ...
            if (err === "INVALID_OTP") return res.render("withdraw-request", { user: user, id: uuid.v4(), warning: "invalid one-time token" });
            //Should be an user
