...
client.query("SELECT COUNT(*) count FROM users WHERE lower(username) = lower($1)", ["harsh"], (err, res) => {
...
exports.getGame = function (gameId, callback) {
    ...
    query(
        "SELECT * FROM games " + "LEFT JOIN game_hashes ON games.id = game_hashes.game_id " + "WHERE games.id = $1 AND games.ended = TRUE",
        [gameId],
        function (err, result) {
            if (err) return callback(err);
...
exports.makeWithdrawal = function (userId, satoshis, withdrawalAddress, withdrawalId, callback) {
    ...
    getClient(function (client, callback) {
        client.query("UPDATE users SET balance_satoshis = balance_satoshis - $1 WHERE id = $2", [satoshis, userId], function (err, response) {
            if (err) return callback(err);

            if (response.rowCount !== 1) return callback(new Error("Unexpected withdrawal row count: \n" + response));

            client.query(
                "INSERT INTO fundings(user_id, amount, bitcoin_withdrawal_address, withdrawal_id) " + "VALUES($1, $2, $3, $4) RETURNING id",
...
exports.setFundingsWithdrawalTxid = function (fundingId, txid, callback) {
    ...
    query("UPDATE fundings SET bitcoin_withdrawal_txid = $1 WHERE id = $2", [txid, fundingId], function (err, result) {
        if (err) return callback(err);
