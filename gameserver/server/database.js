...
exports.endGame = function (gameId, bonuses, callback) {
    ...
    getClient(function (client, callback) {
        client.query("UPDATE games SET ended = true WHERE id = $1", [gameId], function (err) {
...
exports.cashOut = function (userId, playId, amount, callback) {
    ...
    getClient(function (client, callback) {
        addSatoshis(client, userId, amount, function (err) {
            ...
            client.query("UPDATE plays SET cash_out = $1 WHERE id = $2 AND cash_out IS NULL", [amount, playId], function (err, result) {
                if (err) return callback(err);
...
exports.getBankroll = function (callback) {
    query(
        "SELECT (" + "(SELECT COALESCE(SUM(amount),0) FROM fundings) - " + "(SELECT COALESCE(SUM(balance_satoshis), 0) FROM users)) AS profit ",
